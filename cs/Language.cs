...
namespace bygfoot
{
	public class Language
	{
		...
		public static string GetCode()
		{
			...
			string localeOption = Option.OptStr("string_opt_language_code");
...
#endif
			string[] codes = Option.ConstStr ("string_language_codes").Split (new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
			string[] defs = Option.ConstStr ("string_language_defs").Split (new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
			...
			Array.Sort (countryFiles, delegate(string defA, string defB) {
				...
				if (defA.CompareTo(defB) == 0)
					...
					else
					{
						...
						{
							Debug.PrintMessage("Language.CompareCountryFiles: chars {0} and {1} not comparable", defA[i], defB[i]);
							returnValue = 0;
						}
						else
